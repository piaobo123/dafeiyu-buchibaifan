...
export class CompanionReducer {
    ...
    switch (event.type) {
      ...
      case 'tool/call': {
        ...
        if (isUserQuestionTool(name)) {
          ...
          const questionMessage = this.#questionMessage(record, event, callId)
          return questionMessage === undefined ? this.#render() : [...this.#render(), questionMessage]
        }
  ...
  /**
   * Build a QUESTION message for the desktop helper from an ask_user_question
   * tool call. The arguments may be a string (JSON), a plain object, or
   * wrapped in message.content — every shape the model can emit is accepted.
   * @returns a QUESTION companion message, or undefined when no parseable
   *   questions are found (the WAITING state still renders).
   */
  #questionMessage(record, event, callId) {
    ...
    if (!Array.isArray(questions)) {
      // Tool-call arguments may live under message.content in some event shapes.
      const content = event.data?.message?.content
      if (Array.isArray(content)) {
        const block = content.find((item) => Array.isArray(item?.arguments?.questions))
        questions = block?.arguments?.questions
      }
    }
    if (!Array.isArray(questions) || questions.length === 0) return undefined
    return createMessage(CompanionMessageKind.QUESTION, {
      sessionId: record.id,
      callId,
      questions: questions.map((q) => ({
        ...
        ...(Array.isArray(q?.options) && q.options.length > 0
          ...
          : {}),
        ...(q?.multi_select === true ? { multiSelect: true } : {}),
      })),
    })
  }
